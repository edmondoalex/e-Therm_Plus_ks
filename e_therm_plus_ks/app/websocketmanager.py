...
class WebSocketManager:
    ...
    async def fetch_logs(self, items=150):
        if not self._ws or not self._loginId:
            return []
        payload = await getLogs(self._ws, self._loginId, self._logger, items=items)
    ...
    async def schedulers_poller(self):
        ...
        while True:
            ...
            try:
                async with self._ws_lock:
                    payload = await readSchedulers(self._ws, self._loginId, self._logger)
                if not isinstance(payload, dict):
                    await asyncio.sleep(poll_s)
                    continue
    ...
    async def thermostats_cfg_poller(self):
        ...
        while True:
            ...
            try:
                async with self._ws_lock:
                    payload = await readThermostatsCfg(self._ws, self._loginId, self._logger, pin=self._pin)
                if not isinstance(payload, dict):
                    await asyncio.sleep(poll_s)
                    continue
    ...
    async def zones_poller(self):
        ...
        while True:
            ...
            try:
                async with self._ws_lock:
                    payload = await readZones(self._ws, self._loginId, self._logger)
        ...
        while self._retries < self._max_retries:
            try:
                ...
                try:
                    ...
                    therm_payload = await readThermostatsCfg(self._ws, self._loginId, self._logger, pin=self._pin)
            ...
            try:
                async with self._ws_lock:
                    if isinstance(command, dict) and command.get("type") == "PARTITION":
                        ...
                        resp = await writeCfg(
                            self._ws, self._loginId, self._logger, {"CFG_SCHEDULER_TIMERS": [patch]}
                        )
                        ...
                        if ok:
                            ...
                            try:
                                therm_payload = await readThermostatsCfg(self._ws, self._loginId, self._logger, pin=pin)
                                cfg_list = therm_payload.get("CFG_THERMOSTATS") if isinstance(therm_payload, dict) else None
                                if isinstance(cfg_list, list):
                                    if self._readData is None:
                                        self._readData = {}
                        ...
                        pin = command_data.get("pin") or self._pin
                        resp = await clearCmd(self._ws, self._loginId, pin, self._logger, ptype)
                        ok = isinstance(resp, dict) and resp.get("RESULT") == "OK"
                        command_data["future"].set_result(bool(ok))
            except Exception as e:
