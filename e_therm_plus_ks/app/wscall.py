...
"""
async def systemVersion(websocket, login_id, _LOGGER):
    global cmd_id
    cmd_id += 1
...
"""
async def readSchedulers(websocket, login_id, _LOGGER):
    global cmd_id
    cmd_id += 1
    expected_id = str(cmd_id)
    json_cmd = addCRC(
...
"""
async def readZones(websocket, login_id, _LOGGER):
    global cmd_id
    cmd_id += 1
    expected_id = str(cmd_id)
    json_cmd = addCRC(
...
"""
async def readThermostatsCfg(websocket, login_id, _LOGGER, pin: str | None = None):
    global cmd_id
    cmd_id += 1
    expected_id = str(cmd_id)
...
"""
async def getLogs(websocket, login_id, _LOGGER, id_log="MAIN", items=150, items_type=None):
...
"""
async def writeCfg(websocket, login_id, _LOGGER, payload_patch: dict):
...
"""
async def writeCfgTyped(websocket, login_id, _LOGGER, payload_type: str, payload_patch: dict, pin: str | None = None):
    if not isinstance(payload_patch, dict):
        return {}
...
"""
async def clearCmd(websocket, login_id, pin, _LOGGER, payload_type: str):
    global cmd_id
    cmd_id += 1
    expected_id = str(cmd_id)
    ptype = str(payload_type or "").strip().upper()
