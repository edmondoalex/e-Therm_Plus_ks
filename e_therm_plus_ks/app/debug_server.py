...
def render_vtherm_admin(snapshot):
    """Admin page to create and debug vTherm thermostats."""
    entities = snapshot.get('entities') or []
    therms = [e for e in entities if str(e.get('type') or '').lower() == 'thermostats']
    list_items = []
    for e in therms:
...
"""
    ...
    def do_GET(self):
        ...
        if path.startswith("/api/vtherm/config"):
            ...
            data = meta.get("vtherm_config") or {}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .guestLogo { position:absolute; top:18px; left:50%; transform:translateX(-50%); width:min(420px,52vw); aspect-ratio:3/2; opacity:.95; pointer-events:none; background:url("__GUEST_LOGO__") center center / contain no-repeat; filter:drop-shadow(0 12px 32px rgba(0,0,0,.55)); }
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .guestLogo { position:absolute; top:18px; left:50%; transform:translateX(-50%); width:min(420px,52vw); aspect-ratio:3/2; opacity:.95; pointer-events:none; background:url("__GUEST_LOGO__") center center / contain no-repeat; filter:drop-shadow(0 12px 32px rgba(0,0,0,.55)); }
    .page { width:min(520px,100%); display:grid; gap:18px; justify-items:center; text-align:center; padding-top:226px; }
      ...
      <div class="form">
        ...
        <div>
          ...
          <select id="f_src_type">
            ...
            <option value="ha_sensor">ha_sensor</option>
        ...
        <div style="grid-column: 1 / -1;">
          <label>Modo termostato</label>
          <div class="chkRow">
            <div class="chk"><input id="f_display_only" type="checkbox" /> <span>Solo visualizzazione temperatura</span></div>
          </div>
        </div>
        <div style="grid-column: 1 / -1;">
          ...
          <div style="margin-top:8px;">
            <label>Limite +/- gradi guest</label>
            <input id="f_guest_setpoint_offset" placeholder="Default 3" inputmode="decimal" />
          </div>
        </div>
        <div style="grid-column: 1 / -1;">
        ...
        <div>
          <label>Relè Heat/Cool (switch, opzionale)</label>
          <div class="row">
            <input id="f_rt_heat_power_switch" placeholder="Heat: switch.rele_heat" />
            <input id="f_rt_cool_power_switch" placeholder="Cool: switch.rele_cool" />
          </div>
        </div>
        <div>
...
function isHaSensorType(v) {
  const s = String(v || '').trim().toLowerCase();
  return s === 'ha_sensor' || s === 'homeassistant_sensor' || s === 'sensor';
}

function isVirtualSourceType(v) {
  const s = String(v || '').trim().toLowerCase();
  return s === 'virtual' || s === 'local' || s === 'local_virtual';
}

function canonicalSourceType(v) {
  if (isHaMultiAvgType(v)) return 'ha_multi_sensor_avg';
  if (isHaSensorType(v)) return 'ha_sensor';
  if (isVirtualSourceType(v)) return 'virtual';
...
function sanitizeTherm(t) {
  ...
  const guestOffset = Number(String((t && (t.guest_setpoint_offset ?? t.guest_max_offset)) ?? '').replace(',', '.').trim());
  ...
  const base = {
    ...
    ...(guestEnabled && Number.isFinite(guestOffset) && guestOffset > 0 ? { guest_setpoint_offset: guestOffset } : {}),
    ...
    ...(!displayOnly && realTargets ? { real_targets: realTargets } : {}),
    ...(!displayOnly && realTherm ? { real_thermostat: realTherm } : {}),
    ...(climate ? { climate } : {}),
  };
  if (!displayOnly && (outHeat || outCool)) {
...
function renderList() {
  ...
  items.forEach((raw, idx) => {
    ...
    const displayOnly = !!t.display_only;
    ...
    if (displayOnly) {
      chips.innerHTML = '<span class="chip">solo visualizzazione</span>';
    } else if (!split) {
      chips.innerHTML =
...
function toggleDisplayOnlyFields() {
  const on = !!((document.getElementById('f_display_only') || {}).checked);
  const ids = [
    'f_profile', 'f_consensus_group_heat', 'f_consensus_group_cool',
    'f_rt_power_light', 'f_rt_valve_low_switch', 'f_rt_valve_hot_switch',
    ...
    'f_rt_keepalive_sec', 'f_rt_tmin_heat', 'f_rt_tmax_heat', 'f_rt_tmin_cool',
    'f_rt_tmax_cool', 'f_rt_vmc_entity', 'f_rt_vmc_speed', 'f_rt_vmc_off_idle'
  ];
  for (const id of ids) {
    const el = document.getElementById(id);
...
function editItem(idx) {
  ...
  document.getElementById('f_guest_setpoint_offset').value = String((t.guest_setpoint_offset ?? t.guest_max_offset ?? 3));
...
function addNew() {
  ...
  document.getElementById('f_guest_setpoint_offset').value = '3';
  ...
  document.getElementById('f_rt_heat_power_switch').value = '';
  document.getElementById('f_rt_cool_power_switch').value = '';
...
function saveItem() {
  ...
  const guestSetpointOffsetRaw = String(document.getElementById('f_guest_setpoint_offset').value || '').replace(',', '.').trim();
  const guestSetpointOffset = guestSetpointOffsetRaw ? Number(guestSetpointOffsetRaw) : 3;
  ...
  const rtHeatPowerSwitch = String(document.getElementById('f_rt_heat_power_switch').value || '').trim();
  const rtCoolPowerSwitch = String(document.getElementById('f_rt_cool_power_switch').value || '').trim();
  ...
  if (guestEnabled && (!Number.isFinite(guestSetpointOffset) || guestSetpointOffset <= 0)) { if (msg) msg.textContent = 'Limite +/- gradi guest deve essere maggiore di 0.'; return; }
  const climateModes = [];
  ...
  if (!displayOnly && (rtPowerLight || rtValveLow || rtValveHot || rtHeatPowerSwitch || rtCoolPowerSwitch || rtFanMin || rtFanMed || rtFanMax)) {
    realTargets = {};
  ...
  const itemBase = {
    ...
    source: sourceObj,
    ...(displayOnly ? { display_only: true } : {}),
    ...(guestEnabled ? { guest_enabled: true, guest_room: guestRoom, guest_setpoint_offset: guestSetpointOffset } : {}),
...
if (displayOnlySel) displayOnlySel.addEventListener('change', toggleDisplayOnlyFields);
toggleSourceFields();
toggleDisplayOnlyFields();
renderConsensusSelect('f_consensus_group_heat', '');
renderConsensusSelect('f_consensus_group_cool', '');
